# Kimberley van Ruiven

**Ethische AI-consultant · Rotterdam**

Ik bouw AI waar je achter kunt staan: verantwoord, uitlegbaar en met de mens aan het stuur. Voor bedrijven en ondernemers die kiezen voor een mens-gerichte toekomst met technologie.

- MSc Information Studies (UvA), afstudeeronderzoek naar AI-bias in hate speech-detectie (BERT, multi-label classificatie)
- Achtergrond als IT-consultant: softwaretesten, functioneel ontwerp en businessanalyse bij o.a. een bank, een pensioenfonds en een IT-startup
- Nu zelfstandig: GDPR-compliant AI-implementaties, automatisering, webtools en workshops AI-geletterdheid (EU AI Act)

## Uitgelicht werk

| Project | Wat het is |
|---|---|
| [AI CO2-calculator](https://github.com/pastatimes/ai-co2-calculator) | De klimaatimpact van je AI-gebruik in een minuut inzichtelijk (Next.js, Claude API). [Probeer hem live](https://www.kimberleyvanruiven.nl/co2) |
| [Gebruik jij AI, of gebruikt AI jou?](https://github.com/pastatimes/ai-quiz) | Quiz over bewust AI-gebruik (PWA). [Doe de quiz](https://pastatimes.github.io/ai-quiz/) |
| [MSc-onderzoek hate speech-detectie](https://github.com/pastatimes/Master-Thesis-HateSpeech-Twitter) | Multi-label classificatie van politieke tweets met menselijke annotatie (Swivl) en BERT |
| [Vertrouwen in AI: dashboard](https://github.com/pastatimes/ai-dashboard) | PWA-dashboard over verantwoord AI-gebruik (in ontwikkeling) |

## Meer dan AI

- **Web development**: de broncode van mijn eigen site [kimberleyvanruiven.nl](https://github.com/pastatimes/spill-your-tea) (Next.js, TypeScript), inclusief ontwerpiteraties
- **Full-stack leerpad**: Le Wagon bootcamp (Ruby on Rails) en Harvard CS50; de oefenprojecten staan als gearchiveerd leerspoor in mijn repos
- **Kwaliteit en testen**: jaren ervaring als software tester en test consultant in de financiële sector; die blik zit in alles wat ik bouw
- **Gereedschap**: Linux (Ubuntu, zelf geïnstalleerd), Next.js, Vercel, Claude Code, Cursor, VS Code, Git

## Contact

[kimberleyvanruiven.nl](https://www.kimberleyvanruiven.nl) · [info@kimberleyvanruiven.nl](mailto:info@kimberleyvanruiven.nl) · [LinkedIn](https://www.linkedin.com/in/kimberleyvanruiven)

*Hoe ik zelf met AI werk, leg ik volledig open op [kimberleyvanruiven.nl/zo-werk-ik-met-ai](https://www.kimberleyvanruiven.nl/zo-werk-ik-met-ai).*
